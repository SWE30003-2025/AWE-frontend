    ...
    try {
      ...
      const ordersWithInvoices = await Promise.all(
        ordersData.map(async (order) => {
          if (order.invoice) {
            // Use the getOrderInvoice function from api.ts
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        {user && user.wallet !== undefined && !isNaN(Number(user.wallet)) && (
          <div className="text-sm text-gray-600">
            ...
            <span className="font-bold text-green-600">${Number(user.wallet).toFixed(2)}</span>
          </div>
        )}
      </div>
                  ...
                  {order.invoice && (
                    <div className="bg-gray-50 rounded p-4 mb-4">
                      <div className="flex justify-between items-start">
                        <div>
                          <h5 className="font-medium">Invoice: {order.invoice.invoice_number}</h5>
                          <p className="text-sm text-gray-600">
