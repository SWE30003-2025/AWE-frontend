...
export default function AdminAnalytics() {
  ...
  const maxSold = stats.top_products && stats.top_products.length > 0 ? Math.max(...stats.top_products.map(p => p.total_sold)) : 0;
  const averageOrderValue = stats.total_orders > 0 ? stats.total_sales / stats.total_orders : 0;

  return (
    <div className="max-w-6xl mx-auto p-4">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900">Sales Analytics</h1>
        <p className="text-gray-600 mt-2">Overview of your store's performance and top products</p>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-lg">
        <div className="px-6 py-4 border-b border-gray-200">
          ...
          <p className="text-sm text-gray-600">Products ranked by total units sold</p>
        </div>
        <div className="p-6">
          {stats.top_products && stats.top_products.length > 0 ? (
              ...
              {stats.top_products.map((product, index) => (
                <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <h3 className="font-medium text-gray-900">{product.product__name}</h3>
            ...
            <div className="text-center py-8">
              ...
              <h3 className="mt-2 text-sm font-medium text-gray-900">No product data available</h3>
              <p className="mt-1 text-sm text-gray-500">Start selling to see your top products here.</p>
            </div>
          )}
        </div>
      </div>
